...
const float pa_points[NUM_POINTS] = {-800.5, -266.8, 3202.00, 8936.90, 17218.70};
...
void checkSerial() {
  if (Serial1.available() > 0) {
    ...
    if (command == "run0:0") {
      flowmeter0.state = 0;
      Serial1.println("stop"); // Keep for debugging
      ...
      Serial1.print("pv0.txt=\""); // Assuming pv0.txt for flowmeter0
      ...
      flowmeter0.state = 1;
      Serial1.println("run"); // Keep for debugging
    }
    else if (command == "unit0:kPa"){
      flowmeter0.unit = "kPa";
      Serial1.println(flowmeter0.unit);
    }
    ...
    // New commands for flowmeter1
      ...
      flowmeter1.state = 0;
      Serial1.println("stop1"); // Debugging for flowmeter1
      ...
      flowmeter1.state = 1;
      Serial1.println("run1"); // Debugging for flowmeter1
    }
    else if (command == "unit1:kPa"){
      flowmeter1.unit = "kPa";
      Serial1.println(flowmeter1.unit);
    }
